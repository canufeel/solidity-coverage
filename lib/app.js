  ...
  */
  ...
  inSkippedFolder(file){
    ...
    this.skippedFolders.forEach(folderToSkip => {
      const inPackagefolderToSkip = `${this.coverageDir}/contracts/${folderToSkip}`;
      const inNodeModulesPackageToSkip = `${this.coverageDir}/node_modules/${folderToSkip}`;
      if (file.indexOf(inPackagefolderToSkip) === 0 || file.indexOf(inNodeModulesPackageToSkip) === 0)
        shouldSkip = true;
    });
    return shouldSkip;
  }
